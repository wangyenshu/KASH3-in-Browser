...
CONTAINER_NAME=debian-kash3-build
IMAGE_NAME=local/debian-kash3-v86

mkdir -p "$IMAGES"

echo "Building Docker image..."
# &> redirects all output to a log file to keep your terminal clean
docker build . --platform linux/386 --rm --tag "$IMAGE_NAME" 2>&1 | tee build.log | awk 'NR%10==0 {print $0; fflush()}'
